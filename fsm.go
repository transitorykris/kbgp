...
func (f *fsm) reader() {
	...
	case open:
		...
		if _, ok := open.valid(f.peer.remoteAS, durationToUint16(f.holdTime)); !ok {
...
func newFSM(p *peer) *fsm {
	f := &fsm{
		...
		delayOpen:                          false,
		passiveTCPEstablishment:            p.passive,
